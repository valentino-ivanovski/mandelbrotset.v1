...
public class myMandelbrotPreparation extends Application {
    ...
    /* =========================================Variables================================================ */
     ...
     WritableImage imagee = new WritableImage((int) canvas.getWidth(), (int) canvas.getHeight());
     double zoom = 250.0;
    ...
    void MandelbrotSet() {
        ...
        for (int i = 0; i < THREADS; i++) {
            thread[i] = new myMandelbrotParallel(imagee, i, THREADS, width, height, maximumIterations, canvas, zoom, xPos, yPos, hue, brightness, saturation, R, G, B);
            thread[i].start();
        }
        for (int i = 0; i < THREADS; i++) {
            try {
        ...
        canvas.getGraphicsContext2D().drawImage(imagee, 0, 0); //x and y coordinates of the image.
    }
